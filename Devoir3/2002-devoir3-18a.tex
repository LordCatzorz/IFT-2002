...
\begin{tikzpicture}[->,shorten >=1pt,auto,node distance=2cm, semithick]
  ...
  \node[draw=none, align=center] (text) [right =of 5] {
    \textit{Les transitions manquantes}\\
    \textit{s'en vont toutes à l'état rejetant,}\\
    \textit{mais sont omises pour simplifier}\\
    \textit{la présentation de la machine.}\\
  };
...
Au départ, la pile 1 est vide et la pile 2 contient toute la séquence du ruban. L'automate commence par lire sur la pile 2. Si la machine de turing veut faire un déplacement à droite,
...
\noindent Un langage $L$ peut être reconnu par une machine de Turing sans retour si et seulement si $L$ peut-être reconnu par un {\bf automate fini}.
...
Obtenir un iterateur $i$ qui commence à 0. \bigskip

Tant que $i < n$ faire
\begin{itemize}
  \item Pour tous les mots possible de longueur $i$ en utilisant l'alphabet de $M$, vérifier si ce mot est accepté par $M$.
  \item Si le mot est accepté par $M$: $Accepter$.
  \item Sinon, augmenter $i$ de 2 et retourner au début de la boucle.
\end{itemize}

Si la machine ne s'est pas arrêter avec $Accepter$, alors $Refuser$.
...
(3) En utilisant la force brute, tenter de faire une dérivation de $G$ en utilisant au plus $2 |w| - 1$ étapes. Si trouvé, $R_1 = Vrai$ sinon, $R_1 = Faux$.

(4) En utilisant la force brute, tenter de faire une dérivation de $H$ en utilisant au plus $2 |w| - 1$ étapes. Si trouvé, $R_2 = Vrai$ sinon, $R_2 = Faux$.
...
Considérons la fonction $f$ qui étant donné $\langle N, w \rangle$ (où $N$ est une machine de turing et et $w$ est une entrée pour $N$) retourne la description du programme suivant:

\bigskip

\noindent $M_{N,w} = $"Entrée $x$

\indent Si $|x|$ n'est pas divisible par deux alors {\bf rejeter} $x$

\indent Si $|x|$ est divisible par deux alors simuler $N$ sur l'entrée $w$

\indent Si N accepte $w$ alors {\bf accepter} $x$"

\bigskip

Notons que $f$ est calculable.

Si $N$ accepte $w$ alors $M_{N,w}$ n'accepte que les mots de longueur paire, donc $\langle M_{N,w}\rangle$ appartient à $P_{MT}$

Si $N$ n'accepte pas $w$ alors $M_{N,w}$ n'accepte aucun mot, en particulier aucun mot de longueur paire. 

Donc $\langle M_{N,w}\rangle$ n'appartient pas à $P_{MT}$.


\bigskip

Ainsi, $\langle N, w \rangle \in Acc_{MT} \Longleftrightarrow \langle M_{N,w} \rangle \in P_{MT}$ et donc $f$ est une réduction de $Acc_{MT}$ vers $P_{MT}$.

Et puisque $Acc_{MT}$ est indécisable, $P_{MT}$ est également indécidable.
...
EstNonContextuel($L$) $\implies$ EstDecidable($L$) $\implies$ EstDecidable($L^c$)
...
Faux, si nous avons un langage $L_2$ qui refuse toutes les mots de $L_1$, alors en aucun cas la machine $L_1 - L_2$ ne pourra s'arrêter en acceptant.
...
Notons la solution de $a_{max} (n+1)$ $\lambda$

Séparons l'équation en deux partie. $\alpha = a_0 + a_1x + a_2x^2 +\ldots a_{n-1}x^{n-1}$ et $\beta = a_nx^n$ avec $a_n \neq 0$ (Si $a_n = 0$, alors prendre le plus le dernier $a_kx^k$ par ordre croissant de $k$ comme $\beta$)

Si les tous les $a_k$ = $a_n+\psi$ et $\psi > a_n$,

alors nous avons 
$$\alpha = \sum_{i=0}^{n-1} ((a_n + \psi)x)^i $$
$$= (a_n + \psi)\sum_{i=0}^{n-1} (x)^i $$
$$= (a_n + \psi)\frac{1-x^n}{1-x}$$

et
$$\beta = a_nx^n$$

En perdant un peu patiente, nous pourrons comparer les ratios de $\alpha$ sur $\beta$ et trouver qu'avec 
...
Nous trouverons qu'avec (1), $\frac{\alpha}{\beta} > 1$. Qu'avec (2) $\frac{\alpha}{\beta} = 1$ et qu'avec (3), $\frac{\alpha}{\beta} < 1$.

Ceci nous permettera de conclure que dépasser $x = a_{max} (n+1)$, le dernier terme augmentera tellement plus vite que les autres termes que l'équation ne s'approchera plus jamais de 0. La différence continuera de croire pour tout $x > a_{max}(n+1)$

\bigskip

{\bf Réponse 2:}

Étant donné que la réponse 1 nous permet de savoir que $|x| \leq a_{max} (n+1)$, nous pouvons utiliser l'algorithme suivant:

(1) Calculer $a_{max} (n+1)$. L'appeler $y$.

(2) Pour $i = 0$

(3) Si $i > y$, {\bf Refuser} 

(4) Calculer l'équation avec $x = i$. Si l'équation = 0, alors {\bf Accepter}

(5) Calculer l'équation avec $x = -i$. Si l'équation = 0, alors {\bf Accepter}
