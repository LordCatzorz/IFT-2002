...
Soit $L = (Q, \Sigma, \rho, q, F)$, construisons $L_2$ une copie de $L$.

$L_2 = (Q_2, \Sigma, \rho_2, q_2, F_2)$ 

Construisons $L_p$, un $\epsilon$-AFN comme suit:

$L_p = (Q', \Sigma, \rho', q, F_2)$ avec
\begin{itemize}
\item $Q' = Q \cup Q_2 \cup {q'}$ où $q'$ est un nouvel état.
\item $\rho'=
\rho \cup 
\rho_2 \cup 
\{(p, s, q'): p \in F \wedge s \in \Sigma  \} \cup
\{(q', s, q_2): s \in \Sigma\}$ donc, 
\\toutes les transitions de $L$ et $L_2$ 
\\avec une transition de tous les symboles des états finaux de $L$ vers le nouvel état, 
\\et du nouvel état vers l'état inital de $L_2$
\end{itemize}
...
\begin{tikzpicture}[->,shorten >=1pt,auto,node distance=1.5cm, semithick, show background rectangle]
  ...
  \node[state] (8) [right of=7, draw=none] {$L$};
\end{tikzpicture}
\begin{tikzpicture}[->,shorten >=1pt,auto,node distance=1.5cm, semithick, show background rectangle]
  \tikzstyle{every state}=[draw=black,text=black]
  ...
  \node[accepting, state] (6) [below of=2] {};
  \node[state] (7) [below of=3] {};
  \node[state] (8) [right of = 7, draw=none] {$L_2$};
\end{tikzpicture}
...
\begin{tikzpicture}[->,shorten >=1pt, node distance=0.5, auto, semithick, show background rectangle]
  ...
  \node (LN8) [right =of LN7, draw=none] {$L$};
  \node[fit=(LN1)(LN2)(LN3)(LN4)(LN5)(LN6)(LN7)(LN8),draw=black] (LBox) {};

  %q
  \node[state] (q) [right=of LBox] {$q'$};

  %L2
  \node[state,node distance=2] (L2N1) [right =of q] {$q_2$};
  ...
  \node(L2N8) [right =of L2N7, draw=none] {$L_2$};
  \node[fit=(L2N1)(L2N2)(L2N3)(L2N4)(L2N5)(L2N6)(L2N7)(L2N8),draw=black] (L2Box) {};


  \node (1) [right = of L2Box.south east, draw=none] {$L_p$};

  \path
    (LN4) edge[bend left = 55] node {$\forall s \in \Sigma$} (q)
    (LN6) edge[bend right = 55, below] node {$\forall s \in \Sigma$} (q)
    (q) edge node {$\forall s \in \Sigma$} (L2N1)
  ;
